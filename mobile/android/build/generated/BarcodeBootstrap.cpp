...
static Handle<Value> Barcode_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Barcode.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::BarcodeBindings::lookupGeneratedInit(
...
static void Barcode_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Barcode_getBinding)->GetFunction());
}

static void Barcode_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::BarcodeBindings::lookupGeneratedInit(*binding, bindingLength);
	...
	(JNIEnv *env, jobject self)
{
	...
	titanium::KrollBindings::addExternalLookup(&(::BarcodeBindings::lookupGeneratedInit));
}
